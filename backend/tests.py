...
class TestFranchises:
    def test_create_franchise(self):
        ...
        response = client.post("/franchises", json=franchise_data)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_create_duplicate_tax_number(self):
        ...
        client.post("/franchises", json=franchise_data)
        response = client.post("/franchises", json=franchise_data)
    ...
    def test_list_franchises_with_pagination(self):
        response = client.get("/franchises?skip=0&limit=5")
        assert response.status_code == 200
    
    def test_list_franchises_with_search(self):
        response = client.get("/franchises?search=Test")
        assert response.status_code == 200
    
    def test_get_franchise_stats(self):
        response = client.get("/franchises/stats")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_nonexistent_franchise(self):
        response = client.get("/franchises/99999")
